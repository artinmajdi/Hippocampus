function PosteriorComparison = funcVoxelByVoxel3D(FinalData,VoxelByVoxelAnalysisMode)
    ...
    switch VoxelByVoxelAnalysisMode
        case 'OrgansSeperately'
            ...
            NumberOfOrgans     = max(segT1(:));
            

            PosteriorComparison = zeros(y,x,z,NumberOfOrgans,NumberOfModalities);   
            for organInd = 1:NumberOfOrgans

                imT1org  = OrganExtractor(imT1,segT1,organInd);
                imT2org  = OrganExtractor(imT2,segT2,organInd);
                imWMNorg = OrganExtractor(imWMN,segWMN,organInd);

                [A1,A2,A3] = PosterioryEvaluation2(imT1org , imT2org , imWMNorg);
                PosteriorComparison(:,:,:,organInd,1) = A1;
                PosteriorComparison(:,:,:,organInd,2) = A2;
                PosteriorComparison(:,:,:,organInd,3) = A3;
